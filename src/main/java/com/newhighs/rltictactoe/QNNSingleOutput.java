 ...
 * optimal move
 */
public class QNNSingleOutput implements QNN
{
  transient public static final Logger _log = Logger.getLogger(QNNSingleOutput.class);
  ...
  public QNNSingleOutput(int dimension_, int replayMemorySize_)
  {
...
//    _net.setListeners(new HistogramIterationListener(1));
  ...
  public List<Action> argMax(Environment env_, State S_)
  {
    ...
    _log.info("Best: " + best.get(0).encode() + "  " + r);
    return best;
  }

  public QNNSingleOutput load(String filename_) throws IOException
  {
...
//  public QNNSingleOutput copy()
//  {
//    // we can't do deepcopy on the neural net; that's why it's transient
//    QNNSingleOutput copy = JsonConverter.deepCopy(this, QNNSingleOutput.class);
...
//  }
  ...
  public double get(State s_, Action a_)
  {
    ...
    return _net.output(array, false).getDouble(0);
  }
}
